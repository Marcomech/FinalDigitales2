...
// void sigma_delta(unsigned int *wavein, unsigned int *waveout);
...
// 	RCC->CFGR |= (0b0111 << 18);				// Set PLLMULL to 9. multipica el clock 8MHz * 9 = 72 Mhz.
...
//                                                 // No divide la frecuencia para el general
//     RCC->CFGR |= (0b100  << 8);					// Set APB1 PPRE1 division to 2. 
//                                                 // Setea una frec de 72MHz/2= 36 Mhz. Para APB1 
...
// 	DMA1->CHN[CHN2].CNDTR = 2;//N;    // Transfer size
...
// 	TIM2->PSC   = 0;//72e6/(N*8*1e3)-1;	                              // VELOCIDAD DE ENVIO DE DATOS fCK_PSC / (PSC[15:0] + 1)
// 	TIM2->ARR   = 17;//8-1;					                          // AUTO RELOAD. si lo pongo en 1 transfiero todo el tiempo
...
int main(void){   
	//Output C13
	unsigned int data2[2]={0x00000000,0x0000FFFF};
	//unsigned int data[N];
   // unsigned int out_signal[N], out_signal_sig_delta[N];
	...
	for (;;){
		...
		// 	for(int j=0; j <= 3; j++){
		// 		if (GPIOB->IN & (1 << (12+j))){
		// 			out_signal[i] += seno[(i*(1 << j))%N];
		// 		} 
		// 	}
		// 	out_signal[i] = out_signal[i] * MAXVAL/(peak[(GPIOB->IN >> 12)]);
		//	out_signal[i] = seno[i];
		//}

    	//sigma_delta(out_signal, out_signal_sig_delta);

		// for (volatile int i = 0; i < N; i++){
		// 	if(out_signal_sig_delta[i] == 0){
		// 		data[i] = (0x00000000);
		// 	}else{
		// 		data[i] = (0x0000FFFF);
		// 	}
...
//     for (int i = 0; i < N; i++) {
//         if (i > 0) {
//             integrator += ( (wavein[i]+((1-waveout[i-1])*MAXVAL))/2 - HALFMAX);
//         }
//         if (integrator > 0) {
//             waveout[i] = 1;
//         } else {
